...
// Image state
// Legacy single-image globals (kept for export's first-image fallback).
let baseImageTex = null;
let baseImageElement = null;
let hasBaseImage = false;
let imageAspectRatio = 1.0;
let baseImageName = '';
// Per-layer image storage: layerId -> { tex, ar, src, name }. Each image layer
// owns its own WebGL texture so multiple image layers can show different images.
let imageStore = {};
// Which image layer an in-flight file pick targets (set before opening picker).
...
let collapsedZones = new Set();  // right-panel zones the user has collapsed (keyed by label text)
...
// Build/refresh a WebGL texture for one image layer from a loaded <img>.
function buildImageTexture(layerId, img) {
  let rec = imageStore[layerId];
  if (!rec) { rec = imageStore[layerId] = { tex: gl.createTexture(), ar: 1.0, src: '', name: '' }; }
  ...
  rec.ar = img.width / img.height;
  rec.img = img;   // keep the decoded element for offscreen (PNG capture) re-upload
  return rec;
}

// Opens the file picker for a specific image layer.
function chooseImageForLayer(layerId) {
  pendingImageLayerId = layerId;
  const inp = document.getElementById('img-input');
  if (inp) { inp.value = ''; inp.click(); }
}

// File-input handler. Routes the chosen image to the layer that requested it
// (pendingImageLayerId); falls back to the selected/first image layer.
function onImageUpload(input) {
  if (!(input.files && input.files[0])) return;
  let layerId = pendingImageLayerId;
  pendingImageLayerId = null;
  if (layerId == null) {
    const sel = (typeof selectedLayerId === 'number') ? layers.find(l => l.id === selectedLayerId) : null;
    const target = (sel && sel.type === 'image') ? sel : layers.find(l => l.type === 'image');
    if (target) layerId = target.id;
  }
  if (layerId == null) {                       // no image layer yet — create one, then assign
    const lyr = addLayer('image');
    layerId = lyr ? lyr.id : (layers.length ? layers[layers.length - 1].id : null);
  }
  if (layerId != null) assignImageToLayer(input.files[0], layerId);
}

// Reads the file as a data URL (so it persists in saved .frakt files), builds
// the texture, and stores everything on the layer + imageStore.
function assignImageToLayer(file, layerId) {
  const reader = new FileReader();
  reader.onload = () => {
    const dataURL = reader.result;
    const img = new Image();
    img.onload = () => {
      const rec = buildImageTexture(layerId, img);
      const nm = file.name && file.name.length > 20 ? file.name.slice(0,18) + '…' : (file.name || 'image');
      rec.src = dataURL; rec.name = nm;
      // Persist on the layer so Save/Open round-trips the image.
      const layer = layers.find(l => l.id === layerId);
      if (layer) { layer.properties.src = dataURL; layer.properties.imgName = nm; }
      // Keep legacy globals pointing at the most recent image (export fallback).
      baseImageElement = img; baseImageTex = rec.tex; hasBaseImage = true;
      imageAspectRatio = rec.ar; baseImageName = nm;
      renderRightPanel(); needsRecompile = true; snapshot();
    };
    img.src = dataURL;
  };
...
// Rebuild GPU textures for image layers after a scene loads (from saved src).
function restoreImageLayers() {
  (layers || []).forEach(l => {
    if (l.type !== 'image') return;
    const src = l.properties && l.properties.src;
    if (!src) return;
    const img = new Image();
    img.onload = () => {
      const rec = buildImageTexture(l.id, img);
      rec.src = src; rec.name = (l.properties.imgName) || 'image';
      baseImageElement = img; baseImageTex = rec.tex; hasBaseImage = true; imageAspectRatio = rec.ar;
      needsRecompile = true; renderRightPanel();
    };
    img.src = src;
  });
}
...
function renderRightPanel() {
  ...
  attachIroPopovers(panel);
  setupZoneCollapse(panel, 'layer:' + l.id);
}

// Add a chevron and click-to-collapse to every .rp-zone in the right panel.
// State persists in collapsedZones, keyed by `${ctxKey}::${labelText}` so each
// layer (and the frame) remembers its own collapsed sections independently.
// The + button on the Attached Effects header is preserved by skipping clicks
// that originate inside a <button>.
function setupZoneCollapse(panel, ctxKey) {
  panel.querySelectorAll('.rp-zone').forEach(zone => {
    const hdr = zone.querySelector(':scope > .rp-zone-label');
    if (!hdr) return;
    const labelText = (hdr.querySelector(':scope > span')?.textContent || hdr.textContent || '').trim();
    if (!labelText) return;
    const key = ctxKey + '::' + labelText;
    zone.dataset.zone = labelText;
    zone.dataset.zoneKey = key;
    if (!hdr.querySelector(':scope > span')) {
    ...
    if (!hdr.querySelector('.rp-zone-chev')) {
      const chev = document.createElement('span');
      chev.className = 'rp-zone-chev';
      chev.setAttribute('aria-hidden', 'true');
      chev.textContent = '⌄';
      hdr.appendChild(chev);
    }
    hdr.classList.add('rp-zone-label--toggle');
    if (collapsedZones.has(key)) zone.classList.add('rp-zone--collapsed');
    if (hdr.dataset.zoneWired) return;
    hdr.dataset.zoneWired = '1';
    hdr.addEventListener('click', (e) => {
      if (e.target.closest('button')) return;
      const collapsed = zone.classList.toggle('rp-zone--collapsed');
      if (collapsed) collapsedZones.add(key);
      else collapsedZones.delete(key);
    });
  });
}
...
function getPropertyZones(l) {
  ...
  switch (l.type) {
    ...
    case 'image': {
      ...
      const rec = imageStore[id];
      const thumbSrc = (rec && rec.src) || p.src || '';
      const imgName = (rec && rec.name) || p.imgName || 'image';
      const hasImg = !!thumbSrc;
      const dropZone = hasImg
        ? `<div class="img-drop-zone img-drop-zone--thumb has-image" onclick="chooseImageForLayer(${id})">
             <div class="img-thumb"><img src="${thumbSrc}" alt=""></div>
             <div class="img-thumb-meta">
               <div class="img-thumb-name" title="${imgName}">${imgName}</div>
               <div class="img-thumb-action">Click to replace</div>
             </div>
           </div>`
        : `<div class="img-drop-zone" onclick="chooseImageForLayer(${id})">
...
function renderTypeControlsForAttached(layerId, ae) {
  ...
  switch(ae.type) {
    ...
    case 'flow-warp':
      return [
        aeSlider(id,'str','Strength',p.str!=null?p.str:0.3,0,2.0,0.01),
        aeSlider(id,'scale','Scale',p.scale!=null?p.scale:3.0,0.3,8.0,0.1),
        aeSlider(id,'wspd','Speed',p.wspd!=null?p.wspd:0.5,0,3.0,0.01),
        aeSlider(id,'angle','Direction °',p.angle!=null?p.angle:0,0,360,1),
      ].join('');
    case 'ripple':
      return [
        aeSlider(id,'freq','Frequency',p.freq!=null?p.freq:10.0,1.0,40.0,0.5),
        aeSlider(id,'amp','Amplitude',p.amp!=null?p.amp:0.03,0,0.2,0.001),
        aeSlider(id,'decay','Decay',p.decay!=null?p.decay:2.0,0,8.0,0.1),
        aeSlider(id,'cx','Center X',p.cx!=null?p.cx:0.5,0,1.0,0.01),
        aeSlider(id,'cy','Center Y',p.cy!=null?p.cy:0.5,0,1.0,0.01),
      ].join('');
    case 'polar-remap':
      return [
        aeSlider(id,'twist','Twist',p.twist!=null?p.twist:0,-6.28,6.28,0.01),
        aeSlider(id,'zoom','Zoom',p.zoom!=null?p.zoom:1.0,0.1,4.0,0.01),
        aeSlider(id,'cx','Center X',p.cx!=null?p.cx:0.5,0,1,0.01),
        aeSlider(id,'cy','Center Y',p.cy!=null?p.cy:0.5,0,1,0.01),
      ].join('');
    case 'pixelate':
      return aeSlider(id,'size','Block Size',p.size||4,1,64,1);
    case 'chromatic-aberration':
      return [
        aeSlider(id,'spread','Spread',p.spread!=null?p.spread:0.006,0,0.03,0.0005),
        aeSlider(id,'angle','Angle °',p.angle||0,0,360,1),
      ].join('');
    case 'bloom':
      return [
        aeSlider(id,'threshold','Threshold',p.threshold!=null?p.threshold:0.7,0,1.0,0.01),
        aeSlider(id,'strength','Strength',p.strength!=null?p.strength:0.5,0,3.0,0.01),
        aeSlider(id,'radius','Radius',p.radius!=null?p.radius:1.0,0.25,4.0,0.05),
      ].join('');
    case 'glow':
      return [
        aeSlider(id,'threshold','Threshold',p.threshold!=null?p.threshold:0.7,0,1.0,0.01),
        aeSlider(id,'strength','Strength',p.strength!=null?p.strength:0.8,0,3.0,0.01),
        aeSlider(id,'radius','Falloff',p.radius!=null?p.radius:0.5,0.05,2.0,0.01),
      ].join('');
    case 'duotone':
      return aeSlider(id,'blend','Blend',p.blend!=null?p.blend:1.0,0,1.0,0.01);
    case 'n-tone':
      return [
        aeSlider(id,'bands','Bands',p.bands||4,1,8,1),
        aeSlider(id,'mix','Mix',p.mix!=null?p.mix:1.0,0,1.0,0.01),
      ].join('');
    default:
      return '';
...
// ── FTUX onboarding (3-slide) ──────────────────────────────────
// Slide 1 = welcome, slide 2 = how it works, slide 3 = preset gallery.
// Shown on every load unless localStorage.frakt_skip_ftux is set.
// ============================================
// Change this filename to use a different preset
// as the shader on slide 1 of the FTUX modal.
// Must match a file in /presets/ exactly.
// ============================================
const FTUX_SLIDE_1_PRESET = 'Liquid Prism.frakt';

let ftuxSlide = 1;
const FTUX_SLIDES = 3;

function openFtux() {
  const overlay = document.getElementById('ftux-overlay');
  if (!overlay) return;
  ftuxSlide = 1;
  const cb = document.getElementById('ftux-dont-show');
  if (cb) cb.checked = false;              // unchecked every time it opens
  // Re-shuffle the gallery's random 7 on every open
  MODAL_PRESETS = pickModalPresets();
  const slides = document.getElementById('ftux-slides');
  if (slides) slides.style.opacity = '1';
  ftuxRender();
  overlay.classList.remove('hidden');
  buildFtuxShader();                       // live shader on slide 1's left column
}
...
function ftuxRender() {
  for (let i = 1; i <= FTUX_SLIDES; i++) {
    const s = document.getElementById('ftux-slide-' + i);
    if (s) s.classList.toggle('hidden', i !== ftuxSlide);
  }
  document.querySelectorAll('#ftux-dots .ftux-dot').forEach((d, i) => {
    d.classList.toggle('is-active', i === ftuxSlide - 1);
  });
  // Prev chevron is hidden (but kept in place) on the first slide;
  // Next chevron is hidden the same way on the last slide.
  const prev = document.getElementById('ftux-prev');
  if (prev) prev.classList.toggle('is-hidden', ftuxSlide === 1);
  const next = document.getElementById('ftux-next');
  if (next) next.classList.toggle('is-hidden', ftuxSlide === FTUX_SLIDES);
  // Build the live preset gallery lazily, only when slide 3 is shown.
  if (ftuxSlide === FTUX_SLIDES) populateFtuxGallery();
}

function ftuxGo(n) {
  // Wrap forward past the last slide; clamp backward (prev is hidden on slide 1).
  if (n > FTUX_SLIDES) n = 1;
  n = Math.max(1, Math.min(FTUX_SLIDES, n));
  if (n === ftuxSlide) return;
  const slides = document.getElementById('ftux-slides');
  if (!slides) { ftuxSlide = n; ftuxRender(); return; }
  slides.style.opacity = '0';            // fade out
  setTimeout(() => {
    ftuxSlide = n;
    ftuxRender();
...
// Live shader filling slide 1's left column. Uses a REAL preset fetched from
// /presets/ (set via FTUX_SLIDE_1_PRESET) so it authentically represents
// Frakt's output — same fetch + scene pipeline the app uses for presets.
function buildFtuxShader() {
  const cvs = document.getElementById('ftux-shader-canvas');
  if (!cvs) return;
  fetch('/presets/' + FTUX_SLIDE_1_PRESET, { cache: 'no-cache' })
    .then(res => { if (!res.ok) throw new Error('HTTP ' + res.status); return res.json(); })
    .then(data => {
      // Register the fetched scene so the mini-render pipeline can consume it.
      PRESETS['__ftux_slide1'] = frakt2preset(data);
      createMiniRenderer(cvs, '__ftux_slide1');
    })
    .catch(err => console.warn('[ftux] Failed to load slide-1 preset:', FTUX_SLIDE_1_PRESET, err));
}

// Finalize onboarding: honour "don't show again", mark onboarded, run action.
function ftuxFinish(action) {
  const cb = document.getElementById('ftux-dont-show');
  if (cb && cb.checked) localStorage.setItem('frakt_skip_ftux', '1');
  localStorage.setItem('frakt_onboarded', '1');
  closeFtux();
  if (typeof action === 'function') action();
}

// Skip: close, load blank canvas, mark onboarded (does NOT set skip flag).
function ftuxSkip() {
  ftuxFinish(applyBlankCanvas);
}

function populateFtuxGallery() {
  const gallery = document.getElementById('ftux-gallery');
  if (!gallery) return;
  gallery.innerHTML = '';

  // Preset cards (7 random)
  MODAL_PRESETS.forEach(id => {
    ...
    cvs.width = 148; cvs.height = 110;     // render resolution; CSS controls display size
    ...
    card.addEventListener('click', () => {
      ftuxFinish(() => { if (preset) loadScene(preset.raw); });
    });
    gallery.appendChild(card);
    createMiniRenderer(cvs, id);
  });

  // Blank card (final cell)
  ...
  blank.addEventListener('click', () => ftuxFinish(applyBlankCanvas));
  gallery.appendChild(blank);
}

// Legacy welcome-modal alias. The welcome overlay was replaced by the FTUX
// flow; loadPreset()/loadBlank() still call closeModal() as a "hide chrome"
// step, so keep it null-safe.
function closeModal() {
...
function loadScene(data) {
  ...
  applyFrame();
  // Fresh scene → drop old per-layer image textures, then rebuild from saved src.
  imageStore = {};
  hasBaseImage = false; baseImageElement = null; baseImageTex = null;
  restoreImageLayers();
  history = []; historyIdx = -1;
  renderUI(); needsRecompile = true;
  snapshot();
...
// Reset to an empty canvas without touching any modal chrome.
...
// Exports do not carry images — image layers are stripped from the exported
// shader (see stripImageFromShader). Always returns null so no exporter
// embeds an image file or produces a zip.
...
function inlineUniformsInShader(raw, uniforms) {
  ...
  body = body.replace(/^\s*uniform\s+(\w+)\s+([^;\n]+?);\s*$/gm, (m, type, names) => {
    ...
    const out = nameList.map(name => {
      // Leave the ones we WANT to keep as uniforms alone. Image uniforms are
      // now per-layer (uImage_<id>, uHasImage_<id>, uImgAr_<id>) — keep any.
      if (name === 'u_t' || name === 'u_res' ||
          name.indexOf('uImage') === 0 || name.indexOf('uHasImage') === 0 || name.indexOf('uImgAr') === 0) {
...
function stubTexture2DCalls(src, samplerName, stub, prefixMatch) {
  ...
  while (i < src.length) {
    ...
    const samMatch = prefixMatch ? sam.indexOf(samplerName) === 0 : sam === samplerName;
    if (!samMatch) { out += src.slice(idx, k); i = k; continue; }
...
function transformShaderForShadertoy(raw, uniforms) {
  ...
  let body = raw
    ...
    .replace(/^\s*uniform\s+sampler2D\s+uImage\w*\s*;\s*$/gm, '')
    .replace(/^\s*uniform\s+float\s+uHasImage\w*\s*;\s*$/gm, '')
    .replace(/^\s*uniform\s+float\s+uImgAr\w*\s*;\s*$/gm, '')
    .replace(/\buHasImage\w*\b/g, '0.0')
    .replace(/\buImgAr\w*\b/g, '1.0');
  body = stubTexture2DCalls(body, 'uImage', 'vec4(0.2,0.2,0.2,1.0)', true);
...
function bakeShaderForStandalone(raw, uniforms) {
  // Images are not supported in exports — strip every per-layer image uniform
  // (uImage_<id>, uHasImage_<id>, uImgAr_<id>) and neutralize image samples so
  // the exported shader self-contains with no texture dependency.
  const stripped = stripImageFromShader(raw);
  const { body, arrayInits } = inlineUniformsInShader(stripped, uniforms);
  if (!arrayInits.length) return body;
  // Inject array inits right inside main() { ... }
  return body.replace(/void\s+main\s*\(\s*\)\s*\{/, m => m + '\n' + arrayInits.join('\n') + '\n');
}

// Remove all image uniforms/samples from a shader (exports carry no images).
function stripImageFromShader(raw) {
  let body = raw
    .replace(/^\s*uniform\s+sampler2D\s+uImage\w*\s*;\s*$/gm, '')
    .replace(/^\s*uniform\s+float\s+uHasImage\w*\s*;\s*$/gm, '')
    .replace(/^\s*uniform\s+float\s+uImgAr\w*\s*;\s*$/gm, '')
    .replace(/\buHasImage\w*\b/g, '0.0')
    .replace(/\buImgAr\w*\b/g, '1.0');
  return stubTexture2DCalls(body, 'uImage', 'vec4(0.0)', true);
}
...
function tplVanillaIndexHtml(name, w, h, bakedShader, imageSrc, radiusPx) {
  ...
  (function loadImg(){
    ...
    img.onload = function(){
      ...
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
      // Bind this image to every per-layer image sampler (uImage_<id>). A
      // single embedded image is reused across all image layers in the export.
      ...
      for (var ui = 0; ui < n; ui++) {
        ...
        if (nm.indexOf('uImage') === 0)        gl.uniform1i(gl.getUniformLocation(prog, nm), 0);
        else if (nm.indexOf('uHasImage') === 0) gl.uniform1f(gl.getUniformLocation(prog, nm), 1);
        else if (nm.indexOf('uImgAr') === 0)    gl.uniform1f(gl.getUniformLocation(prog, nm), ar);
      }
    };
...
<!--
  ...
  The runtime expects two host-driven uniforms to exist in the shader:
...
function tplThreeIndexHtml(name, w, h, bakedShader, imageSrc, radiusPx) {
  // Per-layer image uniforms are baked into the shader as uImage_<id> etc.
  const lastTok = s => s.replace(/.*\s/, '');
  const imgSamplers = (bakedShader.match(/uniform\s+sampler2D\s+uImage\w*/g) || []).map(lastTok);
  const hasFlags    = (bakedShader.match(/uniform\s+float\s+uHasImage\w*/g) || []).map(lastTok);
  const arFlags     = (bakedShader.match(/uniform\s+float\s+uImgAr\w*/g) || []).map(lastTok);
  const imgBlock = imageSrc ? `
      ...
      imgLoader.load(${JSON.stringify(imageSrc)}, (tex) => {
        ...
        const _ar = tex.image.width / tex.image.height;
${imgSamplers.map(n => `        if (material.uniforms.${n}) material.uniforms.${n}.value = tex;`).join('\n')}
${hasFlags.map(n => `        if (material.uniforms.${n}) material.uniforms.${n}.value = 1;`).join('\n')}
${arFlags.map(n => `        if (material.uniforms.${n}) material.uniforms.${n}.value = _ar;`).join('\n')}
...
<script type="module">
  ...
  const material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      u_t:        { value: 0 },
      u_res:      { value: new THREE.Vector2(W, H) },
${imgSamplers.map(n => `      ${n}: { value: null },`).join('\n')}
${hasFlags.map(n => `      ${n}: { value: 0 },`).join('\n')}
${arFlags.map(n => `      ${n}: { value: 1.0 },`).join('\n')}
    }
  });
${imgBlock}
...
function closeAllOverlays() {
  ...
  const fx = document.getElementById('ftux-overlay');
  if (fx && !fx.classList.contains('hidden')) { ftuxSkip(); closed = true; }
...
function buildCommands() {
  ...
  // View
  cmds.push({ group: 'View', label: 'Open preset gallery',                                 keywords: 'presets gallery start',    run: openFtux });
...
document.addEventListener('keydown', e => {
  ...
  // Plain shortcuts that must never fire while typing in a field.
  if (!mod && !e.altKey && !isTypingInField()) {
    // Space — Play / Pause
    if (e.key === ' ' || e.code === 'Space') { e.preventDefault(); togglePlay(); return; }
    // ? — Show keyboard shortcuts
    if (e.key === '?') { e.preventDefault(); openHelpModal('help-shortcuts-overlay'); return; }
    // ↑ / ↓ — select previous / next layer
    if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
      if (Array.isArray(layers) && layers.length) {
        let idx = layers.findIndex(l => l.id === selectedLayerId);
        if (idx === -1) idx = (e.key === 'ArrowUp') ? layers.length : -1;
        idx = Math.max(0, Math.min(layers.length - 1, idx + (e.key === 'ArrowUp' ? -1 : 1)));
        e.preventDefault();
        selectLayer(layers[idx].id);
      }
      return;
    }
  }
...
// FTUX navigation wiring. The onboarding overlay is intentionally NOT
// dismissed on backdrop click — the user must Skip or pick a starting point.
(function wireFtux() {
  const next = document.getElementById('ftux-next');
  const prev = document.getElementById('ftux-prev');
  const skip = document.getElementById('ftux-skip');
  // Slides change only on manual chevron click — no timers.
  if (next) next.addEventListener('click', () => ftuxGo(ftuxSlide + 1));
  if (prev) prev.addEventListener('click', () => ftuxGo(ftuxSlide - 1));
  if (skip) skip.addEventListener('click', ftuxSkip);
})();
...
// ── Boot ───────────────────────────────────────────────────────
// On mobile (.frakt-mobile set by the detection script in index.html) the
// desktop editor stays dormant — no render loop, no FTUX. The mobile
// experience (lib/mobile.js) loads presets and runs its own hero carousel.
if (!document.documentElement.classList.contains('frakt-mobile')) {
  window.FRAKT_DESKTOP_BOOTED = true;
  noiseTex = initNoiseTex(gl);
  applyFrame();
  renderShortcutHints();
  requestAnimationFrame(frame);
  // Presets live as .frakt JSON files; fetch them all before showing the
  // onboarding modal so the preview cards have scenes to render.
  // Returning users who opted out of the FTUX skip straight to a blank canvas.
  loadAllPresets().then(() => {
    if (localStorage.getItem('frakt_skip_ftux')) {
      applyBlankCanvas();
    } else {
      openFtux();
    }
