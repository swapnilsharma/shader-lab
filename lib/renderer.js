...
function glslImageFn(id) {
  ...
  float iAR=max(uImgAr_${id},0.0001);
  vec2 iuv;
  ...
  layers.forEach(l => {
    ...
    switch(l.type) {
      ...
      case 'image':
        ...
        // Per-layer image so multiple image layers each get their own texture.
        s += `uniform sampler2D uImage_${id};\nuniform float uHasImage_${id};\nuniform float uImgAr_${id};\n`;
        break;
      case 'noise-warp':
...
function buildFragFromLayers(layers, frameState) {
  ...
  // Image uniforms are now declared per-layer in glslUniformDecls (uImage_<id>,
  // uHasImage_<id>, uImgAr_<id>) so multiple image layers don't share one texture.
...
// ── Uniform Setter ─────────────────────────────────────────────
function setUniformsForLayers(glCtx, glProg, layerArr, frameState, t, nt, baseImgTex, hasImg, imgAr, imgResolver) {
  ...
  // Noise texture on unit 1
  glCtx.activeTexture(glCtx.TEXTURE1);
  if (nt) glCtx.bindTexture(glCtx.TEXTURE_2D, nt);

  // Per-layer image textures — each image layer gets its own texture on its
  // own unit (starting at 2), so multiple image layers no longer collide.
  // imageStore is a global owned by the engine (id -> { tex, ar }).
  // imgResolver lets a non-main context (e.g. PNG capture) supply its own
  // per-layer textures; default reads the engine's global imageStore.
  const _imgStore = (typeof imageStore !== 'undefined') ? imageStore : null;
  let _imgUnit = 2;
  (layerArr || []).forEach(l => {
    if (!l || l.type !== 'image') return;
    const rec = imgResolver ? imgResolver(l.id) : (_imgStore ? _imgStore[l.id] : null);
    const uImg = ul('uImage_' + l.id);
    const uHas = ul('uHasImage_' + l.id);
    const uAr  = ul('uImgAr_' + l.id);
    if (rec && rec.tex) {
      glCtx.activeTexture(glCtx.TEXTURE0 + _imgUnit);
      glCtx.bindTexture(glCtx.TEXTURE_2D, rec.tex);
      if (uImg) glCtx.uniform1i(uImg, _imgUnit);
      if (uHas) glCtx.uniform1f(uHas, 1.0);
      if (uAr)  glCtx.uniform1f(uAr, rec.ar || 1.0);
      _imgUnit++;
    } else {
      if (uImg) glCtx.uniform1i(uImg, 0);
      if (uHas) glCtx.uniform1f(uHas, 0.0);
      if (uAr)  glCtx.uniform1f(uAr, 1.0);
    }
  });
...
` + THUMB_TAIL,
  ...
  // ── CONTENT (added later) ───────────────────────────────────
  'linear-gradient': THUMB_HEAD + `
  float g = uv.y;
  vec3 col = mix(vec3(0.97,0.45,0.3), vec3(0.25,0.35,0.9), g);
  col *= 0.92 + 0.08*sin(t);
` + THUMB_TAIL,

  'radial-gradient': THUMB_HEAD + `
  float d = length((uv-0.5)*vec2(u_res.x/u_res.y, 1.0));
  float g = smoothstep(0.0, 0.72, d);
  vec3 col = mix(vec3(1.0,0.82,0.4), vec3(0.32,0.12,0.5), g);
  col *= 0.94 + 0.06*sin(t*1.2);
` + THUMB_TAIL,

  'noise-field': THUMB_HEAD + `
  float f = 0.55*n2(uv*4.0 + t*0.4) + 0.45*n2(uv*8.0 - t*0.3);
  vec3 col = mix(vec3(0.1,0.15,0.32), vec3(0.6,0.85,0.95), f);
` + THUMB_TAIL,

  // ── EFFECTS (added later) ───────────────────────────────────
  'flow-warp': THUMB_HEAD + `
  float flow = n2(uv*3.0 + vec2(t*0.6, 0.0));
  vec2 fuv = uv;
  fuv.x += 0.14*sin(uv.y*8.0 + t*1.5 + flow*3.0);
  vec3 col = mix(vec3(0.15,0.4,0.78), vec3(0.95,0.55,0.82), fuv.x);
  col *= 0.7 + 0.3*sin(fuv.y*12.0 + t);
` + THUMB_TAIL,

  'polar-remap': THUMB_HEAD + `
  vec2 c = (uv-0.5)*vec2(u_res.x/u_res.y, 1.0);
  float ang = atan(c.y, c.x);
  float rad = length(c);
  float a = sin(ang*6.0 + t*2.0)*0.5 + 0.5;
  vec3 col = mix(vec3(0.1,0.05,0.22), vec3(0.9,0.6,0.95), a*smoothstep(0.5,0.05,rad));
` + THUMB_TAIL,

  'n-tone': THUMB_HEAD + `
  float lum = uv.x*0.7 + 0.3*n2(uv*3.0 + t*0.25);
  lum = clamp(floor(lum*4.0)/3.0, 0.0, 1.0);
  vec3 col = mix(vec3(0.12,0.1,0.32), vec3(1.0,0.72,0.4), lum);
` + THUMB_TAIL,

  glow: THUMB_HEAD + `
  vec2 c = (uv-0.5)*vec2(u_res.x/u_res.y, 1.0);
  float d = length(c);
  float ring = smoothstep(0.045, 0.0, abs(d - 0.18));
  float halo = smoothstep(0.55, 0.0, d);
  vec3 col = vec3(0.03, 0.04, 0.06);
  col += vec3(0.3, 1.0, 0.72) * ring;
  col += vec3(0.2, 0.8, 0.6) * halo * (0.5 + 0.5*sin(t*2.0)) * 0.7;
` + THUMB_TAIL,
};
...
function captureCanvasPNG() {
  ...
  // Re-upload each image layer's source into this offscreen context (textures
  // can't be shared across GL contexts). Keyed by layer id for the resolver.
  const oImgMap = {};
  const _store = (typeof imageStore !== 'undefined') ? imageStore : null;
  if (_store) {
    (layers || []).forEach(l => {
      if (!l || l.type !== 'image') return;
      const rec = _store[l.id];
      if (!rec || !rec.img) return;
      const tex = ogl.createTexture();
      ogl.bindTexture(ogl.TEXTURE_2D, tex);
      ogl.texImage2D(ogl.TEXTURE_2D, 0, ogl.RGBA, ogl.RGBA, ogl.UNSIGNED_BYTE, rec.img);
      ...
      oImgMap[l.id] = { tex, ar: rec.ar || 1.0 };
    });
  }
  ...
  setUniformsForLayers(ogl, op, layers, fs2, t, oNoise, null, false, imageAspectRatio, id => oImgMap[id]);
